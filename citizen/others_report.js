...
document.getElementById("moreOptions").addEventListener("click", () => {
  ...
  const reports = [
    {
      ...
      media: [
        { type: "image", url: "https://via.placeholder.com/300x200?text=Speeding+Car" },
        { type: "video", url: "https://sample-videos.com/video123/mp4/240/big_buck_bunny_240p_1mb.mp4" }
      ],
      location: [23.8103, 90.4125],
      upvotes: 12,
      comments: [
        { user: "Alice", text: "Thanks for reporting!" },
        { user: "Bob", text: "I saw it too yesterday." }
      ]
    },
    {
      id: 2,
      ...
      media: [{ type: "image", url: "https://via.placeholder.com/300x200?text=Illegal+Parking" }],
  ...
  function renderReports(list) {
    ...
    list.forEach(report => {
      ...
      // No prompt now, but toggle comment input below
  ...
  function initMap() {
    ...
    // Heatmap plugin requires leaflet.heat script (load in html)
    const heatPoints = reports.map(r => [...r.location, 0.8]); // lat, lng, intensity
  
    if (window.L && L.heatLayer) {
      L.heatLayer(heatPoints, { radius: 25, blur: 15, maxZoom: 17 }).addTo(map);
    } else {
      // fallback: just markers
      reports.forEach(r => {
